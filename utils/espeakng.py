...
class ESpeakNG(object):
    ...
    def _espeak_exe(self, args, sync=False):
        ...
        self._logger.debug('espeakng: executing %s' % repr(cmd))
        ...
        timer = Timer(2.0, self._timeout, [p])
        res2 = []
        try:
            timer.start()
            ...
            if p.poll() is not None and p.poll() < 0:
                ...
                raise TimeoutError('EspeakNG timeout')
            timer.cancel()
            p.stdout.close()
            if p.stderr:
                p.stderr.close()
    ...
    def synth_wav(self, txt, fmt='txt'):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            self._logger.debug('read %s, got %d bytes.' % (f.name, len(wav)))
    ...
    def g2p(self, txt, ipa=None, tie=None):
        ...
        for line in self._espeak_exe(args, sync=True):
            self._logger.debug(u'line: %s' % repr(line))
    ...
    def voices(self):
        ...
        self._logger.debug('espeakng: voices: %s' % res)
        ...
        for v in res:
            ...
            self._logger.debug('espeakng: voices: parts= %s %s -> %s' % (len(parts), repr(parts), repr(voice)))
            voices.append(voice)
